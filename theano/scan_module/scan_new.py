...
def scan(fn,
    ...
    # picture go through sequences picking up time slices as needed
    # TODO modify doc above
    for i, seq in enumerate(seqs):
    ...
    # raise an error later if the sequences are not long enough )
    ...
    oi_seq_i = 0
    for seq in seqs:
        src_name = getattr(seq['input'], 'name', None)
        for tap in seq["taps"]:
            if src_name:
                oi_seqs[oi_seq_i].name = seq['input'].name + \
                    '[%d:]' % tap

    # Operations below will raise an error if seq is too short for steps
    # while evaluating.
    oi_seqs = [seq[:actual_n_steps] for seq in oi_seqs]
    ...
    # MIT_SOT/SIT_SOT -- provided by the user
    # TODO check n_mit_sot -> n_mit_sot_sis valid, what about
    # n_mit_mot
    n_mit_sot_sis = 0
    mit_sot_ois = []
    mit_sot_iis = []
    mit_sot_direct_cal_slices = []
    mit_sot_ios = []
    ...
    for i, init_out in enumerate(outs_info): # sitsot
        ...
        if taps == [-1]:
            ...
            # TODO actual_arg 必须是根变量（输入变量）?
            # Why we need safe_new?
            ...
            sit_sot_direct_cal_slices.append(actual_arg)
            # TODO May I remove return_steps lines.
            if i in return_steps:
                sit_sot_return_steps[n_sit_sot_sis] = return_steps[i]
            sit_sot_iis.append(arg)
            ...
            if np.any(np.array(taps) > 0):
                ...
                raise ValueError('Can not use future taps of outputs',
                                    init_out)
            # go through the taps
            initlen = abs(np.min(taps))
            mit_sot_tap_array.append(taps)
            # Sequence
            # TODO check initial length == initlen
            # TODO is expand_empty gradable to initial
            mit_sot_ois.append(
                scan_utils.expand_empty(init_out['initial'][:initlen],
                                        actual_n_steps))
    ...
    offset = 0
    # TODO Merge below into 1-step / multi-step condition.
    for idx in xrange(n_mit_sot_sis):
        n_inputs = len(mit_sot_tap_array[idx])
        if n_fixed_steps in [1, -1]:
            # TODO merge mit_sot*/sit_sot*
            _ordered_args[mit_sot_rightOrder[idx]] = \
                            mit_sot_direct_cal_slices[offset:offset + n_inputs]
        else:
            _ordered_args[mit_sot_rightOrder[idx]] = \
                            mit_sot_iis[offset:offset + n_inputs]
        offset += n_inputs

    for idx in xrange(n_sit_sot_sis):
        if n_fixed_steps in [1, -1]:
            _ordered_args[sit_sot_rightOrder[idx]] = \
                                        [sit_sot_direct_cal_slices[idx]]
        else:
            _ordered_args[sit_sot_rightOrder[idx]] = \
                                        [sit_sot_iis[idx]]

    ordered_ii_outs_slices = []
    for ls in _ordered_args: # flatten
        ordered_ii_outs_slices += ls
    if n_fixed_steps in [1, -1]:
        stfn_args = (seq_direct_cal_slices +
                ordered_ii_outs_slices +
                non_seqs)

    else:
        stfn_args = (ii_seqs_slices +
                ordered_ii_outs_slices +
                non_seqs)
    ...
    condition, outputs, updates = scan_utils.get_updates_and_outputs(fn(*stfn_args))
    ...
    # TODO some vars can be put in 'fn' implicitly, they may be inputs
    # or depend on some inputs.
    ...
    # TODO How does inputs are absent happen.
    try:
    ...
    for i, out in enumerate(outs_info):
        if 'taps' in out and out['taps'] != [-1]:
            mit_sot_ios.append(outputs[i])

    # Step 5.2 Outputs with tap equal to -1
    for i, out in enumerate(outs_info):
        if 'taps' in out and out['taps'] == [-1]:
            sit_sot_ios.append(outputs[i])

    # Step 5.3 Outputs that correspond to update rules of shared variables
    # TODO How update rule works, how is gradient caled when shared
    # variables are updated each step, does update happened in step_fn
    # or outer_fn. How to distiguish update in fn code and update rules
    # return by fn.
    givens = OrderedDict()
    n_shared_outs = 0
    shared_ois = []
    ...
    for input in dummy_f.maker.expanded_inputs:
        # TODO :If shared is update in fn code, then make it sitsot.
        # GUESS : In convension, we do not modify shared directly
        # in code, we use updates parameter of gof.function instead.
        if isinstance(input.variable, SharedVariable) and input.update:
            # We need to keep update values of a shared var at each
            # steps to calculate gradient, so we convert shared vars
            # which need to be updated to sitsots.
            new_var = safe_new(input.variable)
            if getattr(input.variable, 'name', None) is not None:
                new_var.name = input.variable.name + '_copy'
            # expandable_types are list of types which is capable to
            # add extra dimemsion, till when I'm writing this code,
            # it only contains TensorType, doesn't include ScalarType.
            if isinstance(new_var.type, ops.expandable_types):
                sit_sot_iis.append(new_var)
                sit_sot_ois.append(
                    scan_utils.expand_empty(
                        tensor.unbroadcast(
                ...
                sit_sot_ios.append(tensor_update)
                ...
                # cal gradient.
                shared_iis.append(new_var)
                shared_ois.append(input.variable)
    ...
    n_nit_sot_sis = 0
    nit_sot_ios = []
    nit_sot_return_steps = OrderedDict() # obsolete
    nit_sot_rightOrder = []
    for i, out in enumerate(outs_info):
        if not 'taps' in out:
            nit_sot_ios.append(outputs[i])
            if i in return_steps:
                nit_sot_return_steps[n_nit_sot_sis] = return_steps[i]
    ...
    # Shared or Constant should not be inputs.
    nonseq_ois += [arg for arg in non_seqs
                        if (not isinstance(arg, SharedVariable) and
                            not isinstance(arg, tensor.Constant))]

    # Step 5.6 all shared variables with no update rules
    # TODO : Confused, code below is not for shared.
    nonseq_iis += [safe_new(arg, '_copy') for arg in non_seqs
                         if (not isinstance(arg, SharedVariable) and
                             not isinstance(arg, tensor.Constant))]

    # TODO : I think inner_args should be key.
    givens.update(OrderedDict(izip(nonseq_ois, nonseq_iis)))
    ...
    if strict:
        ...
        # TODO : Ask what does expanded_inputs do?
        static_shared_ois = [arg.variable for arg
                            in dummy_f.maker.expanded_inputs
                            if (isinstance(arg.variable, SharedVariable) and
                                not arg.update and
                                arg.variable in non_seqs_set)]
        static_shared_iis = [safe_new(arg.variable, '_copy') for arg
                            in dummy_f.maker.expanded_inputs
                            if (isinstance(arg.variable, SharedVariable) and
                                not arg.update and
                                arg.variable in non_seqs_set)]
        ...
        static_shared_iis = [safe_new(arg.variable, '_copy') for arg
                            ...
                            if (isinstance(arg.variable, SharedVariable) and
                                not arg.update)]
    givens.update(OrderedDict(izip(static_shared_ois,
                                   static_shared_iis)))
    ...
    # using the givens
    ##
    # TODO why seq is not classed into mit sit
    iis = (ii_seqs_slices +
                    mit_mot_iis +
                    mit_sot_iis +
                    sit_sot_iis +
    ...
    _ois = ([actual_n_steps] +
            ...
            sit_sot_ois +
            shared_ois +
            [actual_n_steps for x in xrange(n_nit_sot_sis)] +
            static_shared_ois +
            nonseq_ois)
    ...
    mit_sot_outs = remove_dimensions(
        oos[offset:offset + n_mit_sot_sis],
    ...
    offset += n_nit_sot_sis
    for idx, update_rule in enumerate(
                oos[offset:offset + n_shared_outs]):
        update_map[shared_ois[idx]] = update_rule
